app.controller('buyerCtrl', function (serverURL, uiURL, $scope, $http, NgTableParams, $window, SweetAlert) {
    ...
    $scope.getServerURL = function () {
        ...
        var host = hostPort[0];
        var port = hostPort[1];
        $scope.urlServer = uiURL;//protocol + "//" + host + ":" + "8081" + "/";
    }
    ...
    $scope.loadAllBuyers = function () {
        ...
        $http.get($scope.urlServer + "api/masterData/getAllBuyer", config).then(
            function (response) {
    ...
    $scope.addBuyer = function (buyer) {
        ...
        $http.post($scope.urlServer + "api/masterData/addBuyer", buyer, config).then(
            function (response) {
                console.log(response);
                switch (response.data.messageType) {
                    case 'success':
                        $scope.message("SUCCESS", response.data.message, "success");
                        $("#addOrEditBuyer").modal("hide");
                        $scope.loadAllBuyers();
                        break;
                    case 'error':
    ...
    $scope.updateBuyer = function (buyer) {
        ...
        $http.put($scope.urlServer + "api/masterData/updateBuyer", buyer, config).then(
            function (response) {
                console.log(response);
                switch (response.data.messageType) {
                    case 'success':
