app.controller('seasonListCtrl', function (serverURL, uiURL, $scope, $http, NgTableParams, $window, SweetAlert) {
    ...
    $scope.loadAllSeasonList = function () {
        ...
        $http.get($scope.urlServer + "api/masterData/getAllSeasonList", config).then(
            function (response) {
    ...
    $scope.addSeasonList = function (seasonList) {
        var config = {
            headers: {
                'NO-AUTH': 'True',
                'Content-Type': 'application/json',
        ...
        seasonList.status = 'active';
        
        $http.post($scope.urlServer + "api/masterData/addSeasonList", seasonList, config).then(
            function (response) {
                console.log(response);
                switch (response.data.messageType) {
                    case 'success':
                        $scope.message("SUCCESS", response.data.message, "success");
                        $("#addOrEditSeasonList").modal("hide");
                        $scope.loadAllSeasonList();
                        break;
                    case 'error':
    ...
    $scope.editSeasonList = function (seasonList) {
        $scope.seasonList = seasonList;
    }
    $scope.addSeasonListClick = function(){
        $scope.seasonList = {};
    }

    $scope.updateSeasonList = function (seasonList) {
        var config = {
            headers: {
                'NO-AUTH': 'True',
                'Content-Type': 'application/json',
        ...
        $http.put($scope.urlServer + "api/masterData/updateSeasonList", seasonList, config).then(
            function (response) {
                console.log(response);
                switch (response.data.messageType) {
                    case 'success':
