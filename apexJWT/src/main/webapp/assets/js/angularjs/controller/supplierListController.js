app.controller('supplierListCtrl', function (serverURL, uiURL, $scope, $http, NgTableParams, $window, SweetAlert) {
    ...
    $scope.loadAllSupplierList = function () {
        ...
        $http.get($scope.urlServer + "api/masterData/getAllSupplierList", config).then(
            function (response) {
    ...
    $scope.loadInitData = function () {
        $scope.loadAllSupplierList();
    }

    $scope.addSupplierList = function (supplierList) {
        var config = {
            headers: {
                'NO-AUTH': 'True',
                'Content-Type': 'application/json',
        ...
        supplierList.status = 'active';
        
        $http.post($scope.urlServer + "api/masterData/addSupplierList", supplierList, config).then(
            function (response) {
                console.log(response);
                switch (response.data.messageType) {
                    case 'success':
                        $scope.message("SUCCESS", response.data.message, "success");
                        $("#addOrEditSupplierList").modal("hide");
                        $scope.loadAllSupplierList();
                        break;
                    case 'error':
    ...
    $scope.editSupplierList = function (supplierList) {
        $scope.supplierList = supplierList;
    }
    $scope.addSupplierListClick = function(){
        $scope.supplierList = {};
    }

    $scope.updateSupplierList = function (supplierList) {
        var config = {
            headers: {
                'NO-AUTH': 'True',
                'Content-Type': 'application/json',
        ...
        $http.put($scope.urlServer + "api/masterData/updateSupplierList", supplierList, config).then(
            function (response) {
                console.log(response);
                switch (response.data.messageType) {
                    case 'success':
